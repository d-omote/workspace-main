...
class Composit2painting:
    ...
    def __init__(self, content_img_path, painting_img_path, nope_img_path):  
        ...
        """
        self.__cropped_style_img_list = fc_style.getCroppedImgList()
        """
        ...
        for i in range(len(self.__styled_img_list)):
            cv2.imwrite('test'+ str(i) + '.jpg', self.__styled_img_list[i]) 
        ...
        """
        # ↓これは実装すべき?
        self.__composit__face_landmark = self.__content_landmark.copy()
        self.__composit_parts_vertex_list = self.__cropped_parts_vertex_list.copy()        
        """
    ...
    """
    def composit_test(self):
        paint_img_h, paint_img_w = self.__painting_img.shape[:2]
        scaling_m = self.get_scaling_matrix()

        #
        parts_center_x = (self.__cropped_parts_vertex_list[4][0][0] + self.__cropped_parts_vertex_list[4][2][0]) / 2
        parts_center_y = (self.__cropped_parts_vertex_list[4][0][1] + self.__cropped_parts_vertex_list[4][2][1]) / 2
        
        #
        parts_img_h, parts_img_w = self.__cropped_content_img_list[4].shape[:2]
        ...
        #img = cv2.warpAffine(self.__cropped_content_img_list[0], trans_m[:2], (paint_img_w, paint_img_h), self.__painting_img, borderMode=cv2.BORDER_TRANSPARENT)
        
        #
        img = cv2.warpAffine(self.__cropped_content_img_list[4], M[:2], (paint_img_w, paint_img_h), self.__painting_img, borderMode=cv2.BORDER_TRANSPARENT)
    ...
    def get_compit2painting_img(self):
        ...
        for i in range(len(self.__composit_img_list)):
            ...
            """
            if i == 5:
                trans_m = np.array([[1, 0, self.__painting_landmark[30][0] - parts_center_x],
                                    [0, 1, self.__painting_landmark[30][1] - parts_center_y],
                                    [0, 0, 1]], np.float32)                            
            """
